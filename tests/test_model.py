...
class TestClass:
    ...
    def test_error_on_shape_1(self):
        with pytest.raises(
                ValueError, match=r"Expected dim 1 of sample to have shape \{3\}"):
            self.model(torch.rand(1, 6, 224, 224)).shape == (1, 10)

    def test_error_on_shape_2(self):
        with pytest.raises(
                ValueError, match=r"Expected dim 2 of sample to have shape \{224\}"):
            self.model(torch.rand(1, 3, 6, 224)).shape == (1, 10)

    def test_error_on_shape_3(self):
        with pytest.raises(
                ValueError, match=r"Expected dim 3 of sample to have shape \{224\}"):
